...
class MarketTiming(strategy.BacktestingStrategy):
    ...
    def _getRank(self, instrument):
        ...
        price = self._getLastPrice(instrument)
        ...
        ret = None
        lookBack = 20
        adjCloseDS = self.getFeed()[instrument].getAdjCloseDataSeries()
        if len(adjCloseDS) >= lookBack and smas[-1] is not None and smas[-1*lookBack] is not None:
            ret = (adjCloseDS[-1] - adjCloseDS[-1*lookBack]) / float(adjCloseDS[-1*lookBack])
        return ret
    ...
    def _getLastPrice(self, instrument):
        lastBar = self.getFeed().getLastBar(instrument)
        return lastBar.getAdjClose()
    ...
    def _placePendingOrders(self):
        ...
        for instrument in self.__sharesToBuy:
            ...
            if orderSize > 0:
                ...
                lastPrice = self._getLastPrice(instrument)
    ...
    def _logPosSize(self):
        ...
        for instrument in self.getBroker().getPositions():
            posSize = positions[instrument] * self._getLastPrice(instrument) / totalEquity * 100
