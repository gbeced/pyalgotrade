...
class ATRPEventWindow(technical.EventWindow):
    def __init__(self, period, multiple, useAdjustedValues):
        assert(period > 1)
        super(ATRPEventWindow, self).__init__(period)
        self.__multiple = multiple 
        self.__useAdjustedValues = useAdjustedValues
        self.__prevClose = None
        self.__value = None
    ...
    def onNewValue(self, dateTime, value):
        tr = self._calculateTrueRange(value)
        super(ATRPEventWindow, self).onNewValue(dateTime, tr)
        self.__prevClose = value.getClose(self.__useAdjustedValues)
    ...
    def getValue(self):
        if self.__value != None and self.__prevClose != None: 
            return (self.__value / self.__prevClose * 100) * self.__multiple
        else: 
            return self.__value 


class ATRP(technical.EventBasedFilter):
    """Average True Range Percent filter as described in http://stockcharts.com/school/doku.php?id=chart_school:technical_indicators:average_true_range_atr
    ...
    def __init__(self, barDataSeries, period, multiple=1, useAdjustedValues=True, maxLen=None):
        ...
        super(ATRP, self).__init__(barDataSeries, ATRPEventWindow(period, multiple, useAdjustedValues), maxLen)