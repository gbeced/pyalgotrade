...
class Trades:
    def __init__(self, trades, ignoreMultiCurrency):
        ...
        for trade in trades:
            ...
            if self.__ignoreMultiCurrency and trade["primary"] != "Y":
                continue
            tradeObj = Trade(trade)
            self.__trades.append(tradeObj)

            # Store first and last trades.
            if self.__first is None or tradeObj.getId() < self.__first.getId():
                self.__first = tradeObj
            if self.__last is None or tradeObj.getId() > self.__last.getId():
